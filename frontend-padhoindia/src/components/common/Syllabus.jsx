...
const Syllabus = () => {
  return (
    <div className="p-4 sm:p-6 md:p-10 bg-gray-50 min-h-screen">
      ...
      <p className="text-center text-base sm:text-lg font-bold text-yellow-600 mb-10">
        From School to State Level – Unlock Knowledge at Every Step
      </p>
      ...
      <div className="grid gap-6">
        {syllabusData.map((section, index) => (
          <Card
            key={index}
            className="shadow-md border border-blue-600 rounded-xl"
            title={
          ...
          >
            <div className="flex flex-wrap gap-3 mt-4">
              {section.subjects.map((subject, i) => (
                <div
                  key={i}
                  className="px-4 py-2 bg-blue-600 text-white rounded-lg font-medium shadow-sm hover:bg-blue-700 transition duration-200 text-sm sm:text-base"
                >
                  {subject}
                </div>
              ))}
